...
data = pd.read_csv('dset1_all_in_one_HM_2D.csv')
N1_sim = data['N 1']
N2_sim = data['N 2']

min_error = float('inf')
best_al = None
plot1 = open('plot_HM2.txt', 'a')
plot1.truncate(0)
...
for al in numpy.linspace(0.4, 0.4, num=1):
    ...
    correct_ratios = []
    i = 0
    for sd_in in numpy.linspace(0.01, 0.21, num=11):
        for sd_ex in numpy.linspace(0.01, 0.21, num=11):
            cmd = f'./a point -t test1 -dim 2 -al {al} -points 1024 -a 1.000000 -b 0.4 0.4 -dvec 0.2 0.2 -dmat 0.001 0.001 0.001 0.001 -sw {sd_in} {sd_ex} {sd_ex} {sd_in} -sm 0.04 0.06'
            os.system(cmd)
            n = open('test1.N', 'r')
            s = (n.readline()).split(' ')
            print("-----")
    ...
    with open('errors_al_HM2.txt', 'a') as f:
        f.write(str(al)+' ')
        f.write(str(error))
        f.write('\n')

    with open(f'correct_ratios_al_HM2.txt', 'a') as f:
        f.write(str(al)+' ')
