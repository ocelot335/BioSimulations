...
for al in numpy.linspace(0.4, 0.4, num=1):
    ...
    for sm2 in numpy.linspace(0.01, 0.21, num=11):
        for d12 in numpy.linspace(0.0001, 0.0021, num=11):
            cmd = f'./a point -t test1 -dim 2 -al {al} -points 1024 -a 1.000000 -b 0.4 0.4 -dvec 0.2 0.2 -dmat 0.001 {d12} 0.001 0.001 -sw 0.04 0.04 0.04 0.04 -sm 0.04 {sm2}'
            os.system(cmd)
            n = open('test1.N', 'r')
            s = (n.readline()).split(' ')
            print("-----")
    ...
    with open('errors_al_CCTO2.txt', 'a') as f:
        f.write(str(al)+' ')
        f.write(str(error))
        f.write('\n')

    with open(f'correct_ratios_al_CCTO2.txt', 'a') as f:
        f.write(str(al)+' ')
