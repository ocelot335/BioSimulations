...
file_name = "plots_not_died_eq_2d"
...
dat_file_path = paste(file_name, "csv", sep = ".")
died_path = paste("plots_died_eq_2d", "csv", sep = ".")
...
grpah_counter = 1
for (sd_m_2 in seq(0.01, 0.22, 0.06)){
    for(d12 in seq(0.0001, 0.0011, 0.0004)) {
        for (sd_w_in in seq(0.01, 0.22, 0.06)){
            for(sd_w_ex in seq(0.01, 0.22, 0.06)) { 
                ...
                print(paste("sd_m_2: ", sd_m_2, "; d12: ", d12))
                params[[paste("birth_kernel", "y", 2, sep = "_")]] = qnorm(grid1, sd = sd_m_2)
                params[paste("dd", 1, 2, sep = "_")] = d12
                ...
                for (iter in 1:(N)){
                    ...
                    first_species = c()
                    second_species = c()
                    sim = new(poisson_2d_n_species, params);
                    DIED1 = 0
                    DIED2 = 0
                    for (k in 1:(Iterations)){
                        sim$run_events(Intensity)
                        popul = sim$total_population()
                        first_species = c(first_species, popul[1])
                        second_species = c(second_species, popul[2])
                        print(popul)
                    ...
                    if(DIED1 ==1 || DIED2 ==1) {
                        ...
                        data = c(data, sd_m_2)
                        data = c(data, d12)
                        cat(data, sep = ";", file = died_path, append = TRUE, fill = FALSE);
                        cat('\n', file = died_path, append = TRUE, fill = FALSE)
                        cat(first_species, sep = ";", file = died_path, append = TRUE, fill = FALSE);
                        cat('\n', file = died_path, append = TRUE, fill = FALSE)
                        cat(second_species, sep = ";", file = died_path, append = TRUE, fill = FALSE);
                        cat('\n\n', file = died_path, append = TRUE, fill = FALSE)
                    } else {
                        data = c()
                        data = c(data, sd_w_in)
                        ...
                        cat(data, sep = ";", file = dat_file_path, append = TRUE, fill = FALSE);
                        cat('\n', file = dat_file_path, append = TRUE, fill = FALSE)
                        cat(first_species, sep = ";", file = dat_file_path, append = TRUE, fill = FALSE);
                        cat('\n', file = dat_file_path, append = TRUE, fill = FALSE)
                        cat(second_species, sep = ";", file = dat_file_path, append = TRUE, fill = FALSE);
                        cat('\n\n', file = dat_file_path, append = TRUE, fill = FALSE)
                    }
                    if(DIED1 == 0) {
                        DIED1 = 1
