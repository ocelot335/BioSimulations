...
file_name = "dset1_all_in_one_CCTO_2D"
...
dd_temp = c()
gridw = linespec(0, 5*SIGMA_W, x_grid_points)
...
for (i in 1:(SPECIES_COUNT)){
    ...
    for (j in 1:SPECIES_COUNT) {
        ...
        params[[paste("death_kernel", "y", i, j, sep = "_")]] = dnorm(gridw, sd = SIGMA_W)
...
write_data = function(sd_m_2, d12, N1, N2, TIME1, TIME2) {
...
seed_counter = 1
for (sd_m_2 in seq(0.01, 0.22, 0.02)){
    for(d12 in seq(0.0001, 0.0022, 0.0002)) {
        print("---")
        ...
        write_data(sd_m_2, d12, N1SUM/N, N2SUM/N, TIME1SUM/N, TIME2SUM/N)
    }
}
