...
header = ""
header = paste(header, "sd_d_in",";")
header = paste(header, "sd_d_ex",";")
header = paste(header, "sd_m_2",";")
header = paste(header, "d12",";")

header = paste(header, "N1",";")
header = paste(header, "N2",";")
...
grpah_counter = 1
for (sd_m_2 in seq(0.13, 0.22, 0.01)){
    for(d12 in seq(0.0001, 0.0011, 0.0001)) {
            ...
            for(sd_w_ex in seq(0.01, 0.22, 0.02)) { 
                ...
                for (iter in 1:(N)){
                    ...
                    for (k in 1:(Iterations)){
                        ...
                        print(popul)
                        if((popul[1] < 100 && popul[2] > 1000 || popul[1] == 0)&& DIED1 == 0) {
                            DIED1 = 1
                            TIME1SUM = TIME1SUM + k*Intensity
                            break
                        }
                        if((popul[2] < 100 && popul[1] > 1000 || popul[2] == 0) && DIED2 == 0) {
                            DIED2 = 1
                    ...
                    data = c(data, popul[1])
                    data = c(data, popul[2])
                    data = c(data, TIME1SUM)
                    data = c(data, TIME2SUM)
                    cat(data, sep = ";", file = dat_file_path, append = TRUE, fill = FALSE);
                    cat('\n', file = dat_file_path, append = TRUE, fill = FALSE)
                }
